  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (client) {
        ...
        console.log(`Checking if vault exists for collection ${collectionMint.toString()}: ${exists}`)
        if (exists) {
          // Temporarily allow creation even if vault exists (for testing with new program ID)
          console.log('Vault exists but allowing creation for testing...')
        ...
        <div className="space-y-8">
          ...
          {client && (
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-6">
              <h4 className="text-blue-400 font-semibold mb-2">Debug: Test PDA Derivation</h4>
              <button 
                onClick={async () => {
                  try {
                    const testCollectionMint = new PublicKey('11111111111111111111111111111111');
                    console.log('Testing PDA derivation with collection mint:', testCollectionMint.toString());
                    const exists = await client.vaultExists(testCollectionMint);
                    console.log('Vault exists for test collection:', exists);
                    alert(`Test completed. Check console for details. Vault exists: ${exists}`);
                  } catch (err) {
